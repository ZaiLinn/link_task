...
class MySettlementDate:
    async def get_settlement_list(self,user,state,OFFSET, LIMIT):
        date =await db.aselect("SELECT task_settlement.id,task_list.title,SUM(task_log.price) AS log_price"
                        ...
                        f"WHERE task_receive.user_id=%s AND task_settlement.state=%s AND task_log.state=0  "
    ...
    async def get_settlement_count(self,user):
        date = await db.aselect("SELECT COALESCE(COUNT(DISTINCT CASE WHEN ts.state = 6 THEN ts.id END), 0) as ban_count,"
                         ...
                         "COALESCE(COUNT(DISTINCT CASE WHEN ts.state = 5 THEN ts.id END), 0) as bans_price_count  "
    ...
    async def get_settlement_log_list(self,settlement,state,OFFSET, LIMIT):
        DB = await db.aselect("SELECT task_log.id,task_log.price,task_log.state,task_log.creation_time,task_user.last_name,task_user.first_name,task_user.username,task_log.label"
                       " FROM task_log  LEFT JOIN task_user ON task_user.id = task_log.in_user WHERE task_log.settlement_id=%s AND task_log.state=%s ORDER BY id DESC LIMIT %s  OFFSET %s",(settlement,state,LIMIT,OFFSET))
        if DB:
            return DB
        return []
    ...
    async def get_settlement_info(self, settlement):
        DB = await db.aselect(
            "SELECT task_settlement.*,task_list.title,task_list.unit_price,group_list.title,group_list.url,chat_user.chat_id,chat_user.full_name,"
        ...
        return DB[0]
    async def get_settlement_one(self,sett):
        DB =  await db.aselect("SELECT * FROM task_settlement WHERE id=%s",(sett,))
        if DB:
            return DB[0]
        return []
    async def put_settlement_appeal(self,sett):
        date = await db.aupdate("UPDATE task_settlement SET appeal=1 WHERE id=%s",(sett,))
        return date
...
class SettlementDate:
    async def get_settlement_list(self,user,state,OFFSET, LIMIT):
        data = await db.aselect("SELECT task_settlement.id,task_settlement.settlement_time,task_list.title,chat_user.full_name,SUM(task_log.price) AS t_price FROM task_settlement "
                         ...
                         "LEFT JOIN task_log ON task_log.settlement_id=task_settlement.id "
                         "WHERE task_settlement.user_id=%s AND task_settlement.state=%s  AND task_log.state=0  "
                         "GROUP BY task_settlement.id ORDER BY task_settlement.id DESC,t_price DESC "
                         "LIMIT %s  OFFSET %s",
                         (user,state,LIMIT,OFFSET))
        if data:
            return data
    ...
    async def get_settlement_count(self,user):
        DB = await db.aselect("SELECT COALESCE(COUNT(DISTINCT ts.id), 0) as all_count,"
                       ...
                       "COALESCE(SUM(CASE WHEN ts.state = 0 THEN tl.price ELSE 0 END), 0) as wait_price_count,"
                       "COALESCE(COUNT(DISTINCT CASE WHEN ts.state = 5 THEN ts.id END), 0) as bans_price_count  "
                       "FROM task_settlement ts LEFT JOIN task_log tl ON ts.id = tl.settlement_id AND tl.state = 0 WHERE ts.user_id = %s",(user,))
        return DB[0]
    async def get_settlement_info(self,settlement):
    ...
    async def get_confirm_info(self,settlement):
        date = await db.aselect("SELECT  task_settlement.id,task_settlement.settlement_time,task_settlement.user_id,chat_user.id,chat_user.chat_id,"
                         "COALESCE(SUM(CASE WHEN task_log.state = 0 THEN task_log.price END), 0) as price_count,task_list.title "
                         "FROM task_settlement "
                         "LEFT JOIN task_receive ON task_receive.id=task_settlement.receive_id "
    ...
    async def put_settlement_info(self,**kwargs):
        ...
        date =await db.aupdate("UPDATE task_settlement SET state=1,pay_order=%s,income_order=%s,label=%s "
                        "WHERE id=%s",(pay_order,income_order,label,settlement))
        return date
    async def put_reject_settlement_info(self,**kwargs):
        settlement = kwargs.get("settlement")
        label = kwargs.get("label")
        date =await db.aupdate("UPDATE task_settlement SET state=5,label=%s "
